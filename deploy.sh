docker build -t bavjackson/multi-client:latest -t bavjackson/multi-client:$SHA ./client/Dockerfile ./client
docker build -t bavjackson/multi-server:latest -t bavjackson/multi-server:$SHA ./server/Dockerfile ./server
docker build -t bavjackson/multi-worker:latest -t bavjackson/multi-worker:$SHA ./worker/Dockerfile ./worker
...
kubectl set image deployments/cilent-deployment client=bavjackson/multi-client:$SHA
